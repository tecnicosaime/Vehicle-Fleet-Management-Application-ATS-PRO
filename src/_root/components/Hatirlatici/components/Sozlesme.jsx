...
const Sozlesme = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`VehicleInfosReminder/GetVehicleContractReminderList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderSozlesme");
    const savedVisibility = localStorage.getItem("columnVisibilitySozlesme");
    const savedWidths = localStorage.getItem("columnWidthsSozlesme");
    ...
    localStorage.setItem("columnOrderSozlesme", JSON.stringify(order));
    localStorage.setItem("columnVisibilitySozlesme", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsSozlesme", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderSozlesme", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilitySozlesme",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderSozlesme");
    localStorage.removeItem("columnVisibilitySozlesme");
    localStorage.removeItem("columnWidthsSozlesme");
