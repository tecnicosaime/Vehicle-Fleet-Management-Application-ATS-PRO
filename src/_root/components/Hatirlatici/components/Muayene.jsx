...
const Muayene = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`VehicleInfosReminder/GetVehicleInspectionReminderList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderMuayene");
    const savedVisibility = localStorage.getItem("columnVisibilityMuayene");
    const savedWidths = localStorage.getItem("columnWidthsMuayene");
    ...
    localStorage.setItem("columnOrderMuayene", JSON.stringify(order));
    localStorage.setItem("columnVisibilityMuayene", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsMuayene", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderMuayene", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityMuayene",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderMuayene");
    localStorage.removeItem("columnVisibilityMuayene");
    localStorage.removeItem("columnWidthsMuayene");
