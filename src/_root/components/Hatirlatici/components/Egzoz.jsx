...
const Egzoz = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`VehicleInfosReminder/GetVehicleExhaustReminderList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderEgzoz");
    const savedVisibility = localStorage.getItem("columnVisibilityEgzoz");
    const savedWidths = localStorage.getItem("columnWidthsEgzoz");
    ...
    localStorage.setItem("columnOrderEgzoz", JSON.stringify(order));
    localStorage.setItem("columnVisibilityEgzoz", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsEgzoz", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderEgzoz", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityEgzoz",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderEgzoz");
    localStorage.removeItem("columnVisibilityEgzoz");
    localStorage.removeItem("columnWidthsEgzoz");
