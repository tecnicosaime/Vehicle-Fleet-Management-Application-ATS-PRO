...
const YakitTuketimi = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`FuelConsumptionReminder/GetFuelConsumptionReminderList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
  ...
  const initialColumns = [
    ...
    {
      title: t("gerceklesen"),
      dataIndex: "gerceklesen",
      key: "gerceklesen",
      width: 120,
      ellipsis: true,
      visible: true, // Varsayılan olarak açık
      sorter: (a, b) => {
        if (a.gerceklesen === null) return -1;
    ...
    {
      title: t("onGorulenMax"),
      dataIndex: "onGorulenMax",
      key: "onGorulenMax",
      width: 120,
      ellipsis: true,
      visible: true, // Varsayılan olarak açık
      sorter: (a, b) => {
        if (a.onGorulenMax === null) return -1;
    ...
    {
      title: t("onGorulenMin"),
      dataIndex: "onGorulenMin",
      key: "onGorulenMin",
      width: 120,
      ellipsis: true,
      visible: true, // Varsayılan olarak açık
      sorter: (a, b) => {
        if (a.onGorulenMin === null) return -1;
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderYakitTuketimi");
    const savedVisibility = localStorage.getItem("columnVisibilityYakitTuketimi");
    const savedWidths = localStorage.getItem("columnWidthsYakitTuketimi");
    ...
    localStorage.setItem("columnOrderYakitTuketimi", JSON.stringify(order));
    localStorage.setItem("columnVisibilityYakitTuketimi", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsYakitTuketimi", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderYakitTuketimi", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityYakitTuketimi",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderYakitTuketimi");
    localStorage.removeItem("columnVisibilityYakitTuketimi");
    localStorage.removeItem("columnWidthsYakitTuketimi");
