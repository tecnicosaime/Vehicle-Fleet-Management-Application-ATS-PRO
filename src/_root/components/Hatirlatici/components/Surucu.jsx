...
const Surucu = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`DriverReminder/GetDriverReminderList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
  ...
  const initialColumns = [
...

    {
      ...
      render: (text, record) => {
        return record.mybVar ? <CheckOutlined style={{ color: "green" }} /> : <CloseOutlined style={{ color: "red" }} />;
      },
      sorter: (a, b) => {
        const aValue = a.mybVar === true ? 1 : 0;
        const bValue = b.mybVar === true ? 1 : 0;
...

    {
      ...
      visible: true, // Varsayılan olarak açık
      render: (text, record) => {
        return record.pskVar ? <CheckOutlined style={{ color: "green" }} /> : <CloseOutlined style={{ color: "red" }} />;
      },
      sorter: (a, b) => {
        const aValue = a.pskVar === true ? 1 : 0;
        const bValue = b.pskVar === true ? 1 : 0;
        return bValue - aValue;
      },
    },

    {
      title: t("mybBitisTarihi"),
      dataIndex: "mybBitisTarih",
      key: "mybBitisTarih",
      width: 110,
      ellipsis: true,
      sorter: (a, b) => {
        if (a.mybBitisTarih === null) return -1;
        if (b.mybBitisTarih === null) return 1;
        return a.mybBitisTarih.localeCompare(b.mybBitisTarih);
      },
    ...
    {
      title: t("srcPiskotektinkBitisTarihi"),
      dataIndex: "srcPiskotektinkBitisTarih",
      key: "srcPiskotektinkBitisTarih",
      width: 110,
      ellipsis: true,
      sorter: (a, b) => {
        if (a.srcPiskotektinkBitisTarih === null) return -1;
        if (b.srcPiskotektinkBitisTarih === null) return 1;
        return a.srcPiskotektinkBitisTarih.localeCompare(b.srcPiskotektinkBitisTarih);
      },
