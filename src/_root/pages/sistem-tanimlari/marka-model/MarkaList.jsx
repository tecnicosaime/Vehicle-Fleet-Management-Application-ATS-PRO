...
const MarkaList = () => {
  ...
  const filteredMarkaList = markaList.filter((item) => item.marka.toLowerCase().includes(searchMarka.toLowerCase()));

  const filteredModelList = modelList.filter((model) => model.modelDef.toLowerCase().includes(searchModel.toLowerCase()));
      ...
      <div className="sistem">
        <Layout style={{ height: "90vh" }}>
          <Sider width={200} style={{ padding: "10px" }}>
            ...
            <Input value={searchMarka} onChange={(e) => setSearchMarka(e.target.value)} placeholder={t("arama")} />
            <Menu mode="inline" style={{ height: "78%", borderRight: 0, overflow: "auto" }} onClick={({ key }) => handleMarkaClick(key)} items={updatedList} />
            <div className="mt-20 flex gap-1 justify-center">
              <Button className="btn primary-btn" onClick={() => setIsOpen(true)}>
                {t("ekle")}
              </Button>
              <Button className="btn primary-btn" onClick={() => setIsUpdateOpen(true)}>
                {t("duzenle")}
              </Button>
            ...
            <Content style={{ padding: 24, margin: 0, minHeight: 280, overflow: "auto" }}>
              {selectedMarka && (
                <>
                  ...
                  <div className="m-20">
                    <Input value={searchModel} onChange={(e) => setSearchModel(e.target.value)} placeholder={t("arama")} />
                  </div>
                  <List
                    dataSource={filteredModelList}
                    renderItem={(model) => (
                      <List.Item onClick={() => handleModelClick(model)} style={{ backgroundColor: selectedModel && selectedModel.siraNo === model.siraNo ? "#e6f7ff" : "#fff" }}>
                        {model.modelDef}
                      </List.Item>
                    )}
              ...
              <div className="model-buttons">
                <Button className="btn primary-btn" onClick={() => setIsModelAddOpen(true)} style={{ marginRight: "8px" }}>
                  {t("ekle")}
                </Button>
                <Button className="btn primary-btn" onClick={() => setIsUpdateModelOpen(true)} style={{ marginRight: "8px" }}>
                  {t("duzenle")}
                </Button>
                <Button className="btn primary-btn" onClick={handleModelDelete}>
                  {t("sil")}
                </Button>
              </div>
            </Content>
