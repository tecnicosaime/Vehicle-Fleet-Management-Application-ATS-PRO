...
const IsKartlari = () => {
  ...
  useEffect(() => {
    if (search.length >= 3) {
      SearchIsKartiListService(tableParams?.pagination.current, search).then((res) => {
        setDataSource(res?.data.list);
        setTableParams({
          ...tableParams,
          pagination: {
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      key: 9,
      render: (_, record) => (
        <Popconfirm title={t("confirmQuiz")} cancelText={t("cancel")} okText={t("ok")} onConfirm={() => handleDelete(record.isTanimId)}>
