...
const Yakit = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.get(`WorkCard/GetWorkCardsList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`);
      ...
      const newData = response.data.list.map((item) => ({
        ...item,
        key: item.isTanimId, // Assign key directly from siraNo
      }));
  ...
  const initialColumns = [
...

    {
      ...
      sorter: (a, b) => {
        if (a.isTip === null) return -1;
        if (b.isTip === null) return 1;
        return a.isTip.localeCompare(b.isTip);
      },
    },
    {
      ...
      sorter: (a, b) => {
        if (a.bakimDepartman === null) return -1;
        if (b.bakimDepartman === null) return 1;
        return a.bakimDepartman.localeCompare(b.bakimDepartman);
      },
    },
...

    {
      ...
      sorter: (a, b) => {
        if (a.ucret === null) return -1;
        if (b.ucret === null) return 1;
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderIsKart");
    const savedVisibility = localStorage.getItem("columnVisibilityIsKart");
    const savedWidths = localStorage.getItem("columnWidthsIsKart");
    ...
    localStorage.setItem("columnOrderIsKart", JSON.stringify(order));
    localStorage.setItem("columnVisibilityIsKart", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsIsKart", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderIsKart", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityIsKart",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderIsKart");
    localStorage.removeItem("columnVisibilityIsKart");
    localStorage.removeItem("columnWidthsIsKart");
