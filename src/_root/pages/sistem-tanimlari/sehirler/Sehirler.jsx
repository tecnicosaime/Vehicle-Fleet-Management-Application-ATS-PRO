...
const Sehirler = () => {
  ...
  useEffect(() => {
    if (search.length >= 3) {
      SearchTownListService(tableParams?.pagination.current, search).then((res) => {
        setDataSource(res?.data.towns);
        setTableParams({
          ...tableParams,
          pagination: {
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      key: 3,
      render: (_, record) => (
        <Popconfirm title={t("confirmQuiz")} cancelText={t("cancel")} okText={t("ok")} onConfirm={() => handleDelete(record.sehirId)}>
  ...
  return (
    <>
      ...
      <UpdateModal updateModal={updateModal} setUpdateModal={setUpdateModal} setStatus={setStatus} status={status} record={record} />
