...
const Yakit = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(`CostAnalysis/GetCostAnalysisInfoByType?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}&type=11`, body);
  ...
  const initialColumns = [
    ...
    {
      title: t("kullandigiAracSayisi"),
      dataIndex: "kullandigiAracSayisi",
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderMaliyetTip11");
    const savedVisibility = localStorage.getItem("columnVisibilityMaliyetTip11");
    const savedWidths = localStorage.getItem("columnVisibilityMaliyetTip11");
    ...
    localStorage.setItem("columnOrderMaliyetTip11", JSON.stringify(order));
    localStorage.setItem("columnVisibilityMaliyetTip11", JSON.stringify(visibility));
    localStorage.setItem("columnVisibilityMaliyetTip11", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderMaliyetTip11", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityMaliyetTip11",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderMaliyetTip11");
    localStorage.removeItem("columnVisibilityMaliyetTip11");
    localStorage.removeItem("columnWidthsMaliyetTip11");
    window.location.reload();
  };

  const componentTitle = t("surucuBazliMaliyet");
