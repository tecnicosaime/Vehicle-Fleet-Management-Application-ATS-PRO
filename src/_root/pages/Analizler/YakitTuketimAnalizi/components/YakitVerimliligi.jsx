...
const YakitVerimliligi = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(`ModuleAnalysis/FuelAnalysis/GetFuelAnalysisInfoByType?type=10&diff=${diff}&setPointId=${currentSetPointId}`, body);
  ...
  const initialColumns = [
    ...
    },
    {
      ...
      sorter: (a, b) => {
        const valA = a.gerceklesenYakit ?? Infinity;
        const valB = b.gerceklesenYakit ?? Infinity;
        return valA - valB;
      },
    },
    {
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderYakitVerimliligi");
    const savedVisibility = localStorage.getItem("columnVisibilityYakitVerimliligi");
    const savedWidths = localStorage.getItem("columnWidthsYakitVerimliligi");
    ...
    localStorage.setItem("columnOrderYakitVerimliligi", JSON.stringify(order));
    localStorage.setItem("columnVisibilityYakitVerimliligi", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsYakitVerimliligi", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderYakitVerimliligi", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityYakitVerimliligi",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderYakitVerimliligi");
    localStorage.removeItem("columnVisibilityYakitVerimliligi");
    localStorage.removeItem("columnWidthsYakitVerimliligi");
