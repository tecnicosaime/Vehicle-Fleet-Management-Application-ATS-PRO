...
const LokasyonBazindaYakitTuketimleri = () => {
  const {
    control,
    watch,
    setValue,
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      if (diff > 0) {
        // Moving forward
        currentSetPointId = data[data.length - 1]?.tbLokasyonId || 0;
      } else if (diff < 0) {
        // Moving backward
        currentSetPointId = data[0]?.tbLokasyonId || 0;
      } else {
        currentSetPointId = 0;
      }

      const response = await AxiosInstance.post(`ModuleAnalysis/FuelAnalysis/GetFuelAnalysisInfoByType?type=8&diff=${diff}&setPointId=${currentSetPointId}`, body);
      ...
      const newData = response.data.list.map((item) => ({
        ...item,
        key: item.tbLokasyonId, // Assign key directly from siraNo
      }));
  ...
  const initialColumns = [
    ...
    },
    {
      ...
      render: (value, record) => {
        ...
        return <span>{value.toFixed(Number(record?.miktarFormat) || 2)}</span>;
      },
      sorter: (a, b) => {
    ...
    },
    {
      ...
      sorter: (a, b) => {
        const valA = a.toplamMesafe ?? Infinity;
        const valB = b.toplamMesafe ?? Infinity;
        return valA - valB;
      },
    },
    {
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderLokasyonBazindaYakitTuketimi");
    const savedVisibility = localStorage.getItem("columnVisibilityLokasyonBazindaYakitTuketimi");
    const savedWidths = localStorage.getItem("columnWidthsLokasyonBazindaYakitTuketimi");
    ...
    localStorage.setItem("columnOrderLokasyonBazindaYakitTuketimi", JSON.stringify(order));
    localStorage.setItem("columnVisibilityLokasyonBazindaYakitTuketimi", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsLokasyonBazindaYakitTuketimi", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderLokasyonBazindaYakitTuketimi", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityLokasyonBazindaYakitTuketimi",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
    ...
    localStorage.setItem(
      "columnWidthsLokasyonBazindaYakitTuketimi",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderLokasyonBazindaYakitTuketimi");
    localStorage.removeItem("columnVisibilityLokasyonBazindaYakitTuketimi");
    localStorage.removeItem("columnWidthsLokasyonBazindaYakitTuketimi");
