...
const Ceza = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(
        `VehicleFines/GetVehicleFinesList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`,
  ...
  const initialColumns = [
...

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.cezaTuru === null) return -1;
        if (b.cezaTuru === null) return 1;
        return a.cezaTuru.localeCompare(b.cezaTuru);
      },
    },
...

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.cezaPuan === null) return -1;
        if (b.cezaPuan === null) return 1;
        // Convert to numbers for numeric comparison
        const numA = parseFloat(a.cezaPuan);
        const numB = parseFloat(b.cezaPuan);
        return numA - numB;
      },
    },
    ...
    {
      title: t("tebligTarihi"),
      dataIndex: "tebligTarih",
      key: "tebligTarih",
      width: 110,
      ellipsis: true,
      visible: true, // Varsayılan olarak açık
      sorter: (a, b) => {
        if (a.tebligTarih === null) return -1;
        if (b.tebligTarih === null) return 1;
        return a.tebligTarih.localeCompare(b.tebligTarih);
      },
      render: (text) => formatDate(text),
    },

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.cezaMaddesi === null) return -1;
        if (b.cezaMaddesi === null) return 1;
        return a.cezaMaddesi.localeCompare(b.cezaMaddesi);
      },
    },
