  ...
  return (
    <div style={style} className="context-menu" ref={modalRef}>
      <Button onClick={() => setVisible(true)}>
        {t("kmGuncellemeGecmis")}: {rowData?.plaka}
      </Button>
      <Button onClick={() => setIsUpdateModalOpen(true)}>{t("guncelKmDzeltme")}</Button>
      <Button onClick={handleReset}>{t("kmSifirlama")}</Button>
      <Modal title={`${t("kmGuncellemeGecmis")}: ${rowData?.plaka}`} open={visible} onCancel={onClose} maskClosable={false} footer={footer} width={1200}>
        <div onClick={(e) => e.stopPropagation()}>
          <KmHistory data={rowData} setTable={setStatus} />
        </div>
      </Modal>

      <Modal title={t("guncelKmSifirlamaSoru")} footer={resetFooter} open={isModalOpen} onCancel={handleCancel}></Modal>

      <Modal
        title="Güncel Km Düzenle"
        footer={updateFooter}
