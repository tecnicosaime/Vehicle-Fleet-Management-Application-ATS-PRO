...
export default function AracTipiniDegistir({ selectedRows, refreshTableData, disabled, hidePopover }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await AxiosInstance.post(`ModifyType/ModifyVehicleType?typeId=${data.aracTipiID}`, aracIDs);
  ...
  return (
    <div style={buttonStyle}>
      <div style={{ marginTop: "8px", cursor: "pointer" }} onClick={() => setIsModalVisible(true)}>
        {t("aracTipiniDegistir")}
      </div>

      <Modal title={t("aracTipiniDegistir")} open={isModalVisible} onOk={handleSubmit(onSubmit)} onCancel={handleCancel}>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div style={{ width: "100%", maxWidth: "450px" }}>
                <StyledDivBottomLine style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                  <Text style={{ fontSize: "14px" }}>Araç Tipi:</Text>
                  <TipSelectbox />
                </StyledDivBottomLine>
              </div>
            </form>
