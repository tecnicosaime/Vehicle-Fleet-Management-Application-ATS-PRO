...
export default function TipSelectbox({ disabled, fieldRequirements }) {
  const {
    control,
    setValue,
    watch,
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`Code/GetCodeTextById?codeNumber=100`);
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("aracTipi", value ?? null);
                setValue("aracTipiID", value ?? null);
