...
export default function GrupSelectbox({ disabled, fieldRequirements }) {
  const {
    control,
    setValue,
    watch,
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`Code/GetCodeTextById?codeNumber=101`);
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("aracGrubu", value ?? null);
                setValue("aracGrubuID", value ?? null);
