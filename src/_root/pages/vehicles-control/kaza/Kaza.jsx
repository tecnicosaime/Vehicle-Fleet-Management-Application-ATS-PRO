...
const Kaza = () => {
  ...
  const fetchData = async (diff, targetPage) => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(`Accident/GetAccidentsList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`, body.filters?.customfilter || {});
  ...
  const initialColumns = [
...

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.kazaTuru === null) return -1;
        if (b.kazaTuru === null) return 1;
        return a.kazaTuru.localeCompare(b.kazaTuru);
      },
    },

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.kazaSekli === null) return -1;
        if (b.kazaSekli === null) return 1;
        return a.kazaSekli.localeCompare(b.kazaSekli);
      },
    },

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.karsiPlaka === null) return -1;
        if (b.karsiPlaka === null) return 1;
        return a.karsiPlaka.localeCompare(b.karsiPlaka);
      },
    },
...

    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.faturaTutar === null) return -1;
        if (b.faturaTutar === null) return 1;
        // Convert to numbers for numeric comparison
        const numA = parseFloat(a.faturaTutar);
        const numB = parseFloat(b.faturaTutar);
        return numA - numB;
      },
    },
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderKaza");
    const savedVisibility = localStorage.getItem("columnVisibilityKaza");
    const savedWidths = localStorage.getItem("columnWidthsKaza");
    ...
    localStorage.setItem("columnOrderKaza", JSON.stringify(order));
    localStorage.setItem("columnVisibilityKaza", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsKaza", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderKaza", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityKaza",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderKaza");
    localStorage.removeItem("columnVisibilityKaza");
    localStorage.removeItem("columnWidthsKaza");
