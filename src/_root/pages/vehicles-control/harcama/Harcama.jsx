...
const Sigorta = () => {
  ...
  const initialColumns = [
    ...
    },
    {
      ...
      visible: true,
      sorter: (a, b) => {
        if (a.harcama === null) return -1;
        if (b.harcama === null) return 1;
        return a.harcama.localeCompare(b.harcama);
      },
    },
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderHarcama");
    const savedVisibility = localStorage.getItem("columnVisibilityHarcama");
    const savedWidths = localStorage.getItem("columnWidthsHarcama");
    ...
    localStorage.setItem("columnOrderHarcama", JSON.stringify(order));
    localStorage.setItem("columnVisibilityHarcama", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsHarcama", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderHarcama", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityHarcama",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderHarcama");
    localStorage.removeItem("columnVisibilityHarcama");
    localStorage.removeItem("columnWidthsHarcama");
