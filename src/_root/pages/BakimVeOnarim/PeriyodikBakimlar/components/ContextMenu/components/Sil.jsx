...
export default function Sil({ selectedRows, refreshTableData, disabled, hidePopover }) {
  ...
  const handleDelete = async () => {
    let isError = false;
    // Tüm seçili satırların anahtarlarını bir diziye topla
    const body = selectedRows.map((row) => row.key);
    try {
      // Silme API isteğini gönder (tek bir istekte tüm anahtarları gönderiyoruz)
      const response = await AxiosInstance.post(`PeriodicMaintenance/DeletePeriodicMaintenanceById`, body);
