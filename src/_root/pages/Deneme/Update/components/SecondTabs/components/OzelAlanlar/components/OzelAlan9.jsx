...
export default function OzelAlan9({ disabled, fieldRequirements }) {
  const {
    control,
    setValue,
    watch,
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`Code/GetCodeTextById?codeNumber=889`);
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("ozelAlan9", value ?? null);
                setValue("ozelAlan9ID", value ?? null);
