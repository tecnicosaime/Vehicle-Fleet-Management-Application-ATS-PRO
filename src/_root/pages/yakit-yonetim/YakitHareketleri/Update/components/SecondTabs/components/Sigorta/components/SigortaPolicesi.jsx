...
export default function SigortaPolicesi({ workshopSelectedId, onSubmit }) {
  ...
  const [columns, setColumns] = useState(() => {
    const savedWidths = localStorage.getItem("SigortaPolicesiTableColumnWidths");
  ...
  const handleResize =
    ...
    (_, { size }) => {
      ...
      localStorage.setItem("SigortaPolicesiTableColumnWidths", JSON.stringify(newColumns.map((col) => col.width)));
    };
  ...
  const sigortaBilgileri = watch("sigortaBilgileri");
  ...
  return (
    <div>
      <Button onClick={handleModalToggle} disabled={!sigortaBilgileri || !plakaID}>
        +
      </Button>
      <Modal width={1200} centered title="Sigorta Poliçesi" open={isModalVisible} onOk={handleModalOk} onCancel={handleModalToggle}>
        <Input
